...
from root import reverseSlash, standardizeFile
class Tag:
	...
	def removeFromFileList(self, targetFile):
		...
		for file in self.__fileList:
			if standardizeFile(file)==standardizeFile(targetFile):
		...
		if(len(self.__fileList)>0):
			for i in range(0,len(self.__fileList)):
				if(len(self.__fileList[i])>0):
					string="".join([string," ",reverseSlash(self.__fileList[i],"\\")])
		#else:
