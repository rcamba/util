...
from root import standardize_string, switch_board, print_numbered_list, choose_from_numbered_list, set_clipboard_data, compare_lists
...
def preHandleSearchTargetList(searchTargetList):
	...
	if len(zList)==0:
		return compare_lists(findFile(searchTargetList), findFile(zList), similar=False)
	else:
		pass
	"""
	finalPossRes=[]
...
def findFile(searchTargetList):  #"Implement selected directories ?"
	...
	for file in fileList:
		...
		for searchTarget in searchTargetList:
			if standardize_string(searchTarget) in standardize_string(path.split(file)[1]):
				counter=counter+1
...
if __name__ == "__main__":
	...
	switches=switch_board(argv, AVAILABLE_SWITCHES)
	...
	if len(argv)>2 :
		if  path.isdir(argv[1]):
			...
			print_numbered_list(resultList)
			if 's' in switches:
				uSelect=choose_from_numbered_list(resultList)
				set_clipboard_data(uSelect)
